...
│ Sonnet 4.5   │ Code writing, reviews, most work            │
│ Opus 4.6     │ Architecture, complex reasoning, debugging  │
└──────────────┴─────────────────────────────────────────────┘
...
├──────────────────────────────────────────────────────────────┤
│                                                              │
│  sigstudio (M4 Max)                sigserve (M2 Max)         │
│    └─ Primary dev machine          └─ Bot server              │
│    └─ Claude Code sessions          └─ OpenClaw + BlueBubbles │
│    └─ Voice input (Typeless)        └─ Telegram, Slack, iMsg  │
│              │                       └─ Tailscale Funnel      │
│              └──────────┬───────────────────┘                │
│                         │                                    │
│  sigair (MacBook Air)  sigtower (UNRAID)                     │
│    └─ Portable laptop    └─ Media/Docker server              │
│    └─ On-the-go dev      └─ Home Assistant                   │
│                          └─ Ollama (local LLM)               │
│                          └─ Subnet router (unraid-docker)    │
│                          └─ 100.119.19.61                    │
│                            │                                 │
│                          sigpc (Windows)                      │
│                            └─ Office workstation              │
...
All machines use `sig` prefix. SSH config shared across fleet.
Auth via 1Password SSH agent (no local key files).

### Services on sigtower
...
### Services on sigserve

| Service | Port | Purpose |
|---------|------|---------|
| OpenClaw Gateway | 18789 | AI bot gateway |
| BlueBubbles | 1234 | iMessage bridge |
| Memory API | 8100 | Unified memory |
...
*Sigstack is maintained at github.com/willsigmon/sigmachines (private)*
*84+ skills across 12 plugins*
*Updated: February 8, 2026 — v3.1*
